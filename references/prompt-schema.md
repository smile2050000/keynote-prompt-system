...
```text
生成一张完整的 16:9 发布会 PPT 页面。当前 v0.9.3 正式生产使用 Image2 整页生图模式，画面和可见文字一次生成；不得在执行中自动切换为底图排字或其他生产模式。
...
```text
现代高端女性生活方式商业摄影，明亮白色与低饱和浅紫空间，少量银灰或克制腮红色作为重点；柔和日光或柔和影棚主光，轻微辅光与受控轮廓光，清晰中间调、真实材质微纹理、自然接触阴影和受控高光；产品、人物和空间保持清晰层级，留白优雅，禁止梦幻光球、粉紫大渐变、暖黄滤镜、灰雾、过曝、少女化图标、婚礼展台和美妆广告装饰
```
...
- 创建 `qa/qa.md`，包含全页验收账本和问题页处理表。全页验收账本证明每页都完成文字/数字、产品/Reference、Global Visual Contract 和 Storyboard 关系检查；问题详情仍只记录需要处理的页面。
